...
data_loader_train = DataLoader(dataset, batch_size=batch_size,collate_fn= KITTI_collate_fn, num_workers=8, shuffle=True,pin_memory=True)
...
model_dict = torch.load("./weights/model_KITTI_CP2.pth") 
...
def show_model_inference():
    ...
    img2 = cv2.resize(img2,(1242,375))*255.0
    ...
    ax3.imshow(pseudo_img[0,0:64].mean(0).detach().cpu().numpy(),cmap="jet") #From Pillars
    ax3.axis("tight")
    ax3.invert_yaxis()
    
    fig4,ax4 = plt.subplots(figsize=(10,10))
    ax4.imshow(pseudo_img[0,64:].max(0).values.detach().cpu().numpy(),cmap="jet") #From RGB
    ax4.axis("tight")
    ax4.invert_yaxis()
