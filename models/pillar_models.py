...
from detector_models import deform_resnet_backbone
...
class Pseudo_IMG_Scatter(torch.nn.Module):
    ...
    def forward(self, pillars, coord, contains_pillars):
        ...
        for batch_idx, idx1, idx2 in zip(np.arange(batch), interval[:-1], interval[1:], strict=False):
...
class RGB_Net(torch.nn.Module):
    ...
    def forward(self, RGB, dynamic_img, pillar_img_pts, rgb_coors, contains_rgb):
        ...
        for batch_i, (a, b, c, pnts_2d, coors, contain) in enumerate(zip(c3, c4, c5, pillar_img_pts, rgb_coors, contains_rgb, strict=False)):
