...
class Pseudo_IMG_Scatter_Pillar(torch.nn.Module):
    ...
    def forward(
        ...
        dynamic_img = (
            torch.zeros(batch, self.xsize * self.ysize, 3)
            .to(pillars.device)
            .type(pillars.type())
        )
        ...
        for batch_idx, idx1, idx2 in zip(
            ...
            dynamic_img[batch_idx][indices.long(), :] += dynamic_outs.type(
                pillars.type(),
            )

        pseudo_img = pseudo_img.view(batch, self.xsize, self.ysize, n_features).permute(
            0,
            3,
            1,
            2,
        )
        dynamic_img = dynamic_img.view(batch, self.xsize, self.ysize, 3).permute(
            0,
            3,
            1,
            2,
...
class RGB_Net(torch.nn.Module):
    ...
    def forward(
        ...
        pseudo_rgb_imga = (
            torch.zeros(batch, C, self.xsize, self.ysize)
            .to(RGB.device)
            .type(RGB.type())
        )
        pseudo_rgb_imgb = (
        ...
        for batch_i, (a, b, c, pnts_2d, coors, contain) in enumerate(
            ...
            pseudo_rgb_img_filter = (
                torch.ones(1, self.xsize, self.ysize, 2).to(RGB.device).type(RGB.type())
                * -2
            )
            mask = (
                torch.zeros(1, 1, self.xsize, self.ysize)
                .to(RGB.device)
                .type(RGB.type())
            )
            ...
            pseudo_rgb_imga[batch_i] = (
                grid_sample(a.unsqueeze(0), pseudo_rgb_img_filter, align_corners=True)
                * mask
            )
            pseudo_rgb_imgb[batch_i] = (
                grid_sample(b.unsqueeze(0), pseudo_rgb_img_filter, align_corners=True)
                * mask
            )
            pseudo_rgb_imgc[batch_i] = (
                grid_sample(c.unsqueeze(0), pseudo_rgb_img_filter, align_corners=True)
                * mask
            )

        pseudo_rgb_img = (
            (pseudo_rgb_imga * dynamic_img[:, 0:1])
            + (pseudo_rgb_imgb * dynamic_img[:, 1:2])
            + (pseudo_rgb_imgc * dynamic_img[:, 2:3])
        )
...
class NET_4D_EffDet(torch.nn.Module):
    def __init__(
        ...
        self.n_features = n_features
        self.SSD = Efficient_Det(
            anchor_dict,
            n_pnts_features=128,
            n_classes=n_classes,
            xyz_range=xyz_range,
        )
        self.pillar_net = Pillar_Network_SECOND(
            input_features=n_input_features,
            n_features=n_features,
            n_pnts_pillar=n_pnt_pillar,
        )
        self.pillar_to_img = Pseudo_IMG_Scatter_Pillar(
    ...
    def forward(
        ...
        pseudo_img_pillar, dynamic_img = self.pillar_to_img(
            learned_pillars,
            coord,
            contains_pillars,
        )
        pseudo_rgb_img = self.rgb_net(
            img,
            dynamic_img,
            pillar_img_pts,
            rgb_coors,
