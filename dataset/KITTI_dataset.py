...
class KittiDataset(Dataset):
    ...
    def __init__(
        self,
        root: str | Path = '/home/conda/RAID_5_14TB/DATASETS/KITTI_dataset/training/',
        xyz_range: np.ndarray | None = None,
        xy_voxel_size: np.ndarray | None = None,
        points_per_pillar: int = 100,
        n_pillars: int = 12000,
        *,
        ...
        super().__init__()
        xyz_range = (
            np.array([0, -40.32, -2, 80.64, 40.32, 3])
            if xyz_range is None
            else xyz_range
        )
        xy_voxel_size = (
            np.array([0.16, 0.16]) if xy_voxel_size is None else xy_voxel_size
        )

        self.return_calib = return_calib

        self.train = True
        self._build_file_index(root, stx=stx, test=test)
    ...
    @staticmethod
    def transform_labels_into_lidar_coordinates(
        labels: list[Label3D],
        R: np.ndarray,
        t: np.ndarray,
    ) -> list[Label3D]:
    ...
    def _normalize_pillars(self, pillars: np.ndaray) -> np.ndarray:
        ...
        pillars[..., 1] = (pillars[..., 1] - self.xyz_range[1]) / (
            self.xyz_range[4] - self.xyz_range[1]
        )
        pillars[..., 2] = (pillars[..., 2] - self.xyz_range[2]) / (
            self.xyz_range[5] - self.xyz_range[2]
        )
    ...
    def getitem(self, idx: int):
        ...
        pc_filter = (
            ...
            & (pointcloud[:, 0] < self.xyz_range[3])
            & (pointcloud[:, 1] > self.xyz_range[1])
            & (pointcloud[:, 1] < self.xyz_range[4])
            & (pointcloud[:, 2] > self.xyz_range[2])
        ...
        voxels, coors, num_points_per_voxel = points_to_voxel(
            pointcloud,
            voxel_size=[*self.xy_voxel_size, 4],
            coors_range=[*self.xyz_range],
            max_points=self.points_per_pillar,
            max_voxels=self.n_pillars,
        )
        ...
        pillars[: voxels.shape[0], :, 4:5] = voxels[..., 0:1] - (
            (np.expand_dims(coors[..., 1:2], 1)) * self.xy_voxel_size[0] + self.x_offset
        )
        pillars[: voxels.shape[0], :, 5:6] = voxels[..., 1:2] - (
            (np.expand_dims(coors[..., 2:], 1)) * self.xy_voxel_size[1] + self.y_offset
        )
        pillar_means = voxels[..., :3].sum(1) / np.expand_dims(num_points_per_voxel, -1)
        pillars[: voxels.shape[0], :, 6:] = voxels[..., :3] - np.expand_dims(
            pillar_means,
            1,
        )
        ...
        pillar_img_pts = calib.project_velo_to_image(
            pillar_loc_xyz,
        )  # project pillar means to rgb image
        img_pnts_filter = (
            (pillar_img_pts[:, 0] > 0)
            & (pillar_img_pts[:, 0] < W)
            & (pillar_img_pts[:, 1] > 0)
            & (pillar_img_pts[:, 1] < H)
        )  # include only pillar means in photo
        n_rgb = np.sum(img_pnts_filter)
        contains_rgb[:n_rgb] = (
            1  # set bool index to say how many rgb points is in the voxel
        )
        rgb_coors[:n_rgb] = coord[: voxels.shape[0]][
            img_pnts_filter
        ]  # indicate which rgb point corresponds to which pillar
        pillar_img_pts2[:n_rgb] = pillar_img_pts[
            img_pnts_filter
        ]  # scatter the filtered points back to tensor
        pillar_img_pts2 = pillar_img_pts2 / np.array([
            W,
            H,
        ])  # normalize to between 0,1, represents position (x,y) on image

        # 5.Normalize pillars for faster convergence
        pillars = self._normalize_pillars(pillars)
