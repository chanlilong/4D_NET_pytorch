...
try:
    ...
    if torch.__version__ == 'parrots':
        ...
        EXT_TYPE = 'parrots'
    elif (hasattr(torch, 'is_mlu_available') and torch.is_mlu_available()) or os.getenv(
        'FORCE_MLU', '0',
    ) == '1':
...
def parse_requirements(fname='requirements/runtime.txt', with_version=True):
    ...
    def gen_packages_items():
        if pathlib.Path(require_fpath).exists():
...
def get_extensions():
    ...
    if EXT_TYPE == 'parrots':
        ...
        include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common').resolve())
        include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common/cuda').resolve())
        ...
        if os.getenv('MMCV_WITH_DIOPI', '0') == '1':
            ...
            assert digit_version(mmengine.__version__) >= digit_version('0.7.4'), (
                f'mmengine >= 0.7.4 is required \
            ...
            op_files = glob.glob('./mmcv/ops/csrc/pytorch/*.cpp') + glob.glob(
            ...
            if mlu_ops_path:
                ...
                if exists_mluops_version != mmcv_mluops_version:
                    print(
                        'the version of mlu-ops provided is %s,'
                        ' while %s is needed.'
                        % (exists_mluops_version, mmcv_mluops_version),
                    )
                    exit()
                try:
                    if pathlib.Path('mlu-ops').exists():
                        if pathlib.Path('mlu-ops').is_symlink():
                            pathlib.Path('mlu-ops').unlink()
                            os.symlink(mlu_ops_path, 'mlu-ops')
                        elif pathlib.Path('mlu-ops').resolve() != mlu_ops_path:
                            os.symlink(mlu_ops_path, 'mlu-ops')
                    else:
                        os.symlink(mlu_ops_path, 'mlu-ops')
                except Exception:
                ...
                if not pathlib.Path('mlu-ops').exists():
                    import requests
                    ...
                    with ZipFile('./mlu-ops.zip', 'r') as archive:
                        try:
                            ...
                            pathlib.Path(dir_name).rename('mlu-ops')
                        except BadZipFile:
                    ...
                    if exists_mluops_version != mmcv_mluops_version:
                        print(
                            'the version of provided mlu-ops is %s,'
                            ' while %s is needed.'
                            % (exists_mluops_version, mmcv_mluops_version),
                        )
                        exit()
            ...
            mluops_includes = []
            mluops_includes.append('-I' + pathlib.Path('./mlu-ops/bangc-ops').resolve())
            mluops_includes.append(
                '-I' + pathlib.Path('./mlu-ops/bangc-ops/kernels').resolve(),
            )
            extra_compile_args['cncc'] = (
                [mlu_args] + mluops_includes if mlu_args else mluops_includes
            )
            ...
            include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common').resolve())
            include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common/mlu').resolve())
            include_dirs.append(pathlib.Path('./mlu-ops/bangc-ops').resolve())
        elif (
            hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()
        ) or os.getenv('FORCE_MPS', '0') == '1':
            ...
            extension = CppExtension
            include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common').resolve())
            include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common/mps').resolve())
            ...
            include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common').resolve())
            include_dirs.append(pathlib.Path('./mmcv/ops/csrc/common/npu').resolve())
        else:
